...
import { fundTenderly } from './helpers'
...
const mintUSDC = async (destinationAddress: Address) => {
  ...
  if (gatewayAddress.toLowerCase() !== signerAddress.toLowerCase()) {
    await setContractStorage(asset.tokenAddress, USDC_GATEWAY_SLOT, signerAddress.toLowerCase())
    const gatewayAddressAfter = await contract.gatewayAddress()

    if (gatewayAddressAfter.toLowerCase() !== signerAddress.toLowerCase()) {
      throw new Error('Failed to set gateway address')
    }
  }
...
const main = async () => {
  await fundTenderly(ArbitrumGateway)

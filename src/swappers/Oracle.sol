...
contract Oracle is IOracle, Ownable {
    ...
    function getPrice(address from, address to, uint8 decimals) external view override returns (uint256) {
        if (oracles[from] == address(0) || oracles[to] == address(0)) revert TokenNotSupported();

        return uint256(PriceConverter.getDerivedPrice(oracles[from], oracles[to], decimals));
    }
}
